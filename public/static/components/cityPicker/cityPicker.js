...
(function ($, win, doc) {
  var CityPicker = function (el, options) {
    ...
    this.popstate = function () {
      if (that.historyNum === 1) {
        $('.pro-picker').animate({left: '200', opacity: 1}, 500, function () {
          $('.picker-box').hide();
        });
        that._closeEvent.remove();
      } else if (that.historyNum === 2) {
        $('.city-picker').animate({left: '200', opacity: 1}, 500, function () {
          $('.pro-picker').show();
          $('.city-picker').hide();
        });
      } else {
  ...
  p.initEvent = function () {
    this.el.on('click', function (e) {
      ...
      this._closeEvent = this.eventListener(window, 'popstate', this.popstate);
      this.forward();
      if (pickerBox[0]) {
        pickerBox.remove();
        this.create();
  ...
  p.create = function () {
    ...
    this.createNavBar();
    this.proPickerEvent();
    this.cityPickerEvent();
    this.navEvent();
  };

  p.createCityPickerBox = function () {
    var proBox = '<div class="picker-box animated bounceInLeft"></div>';
    $('body').append(proBox);
  };
  ...
  p.createCityList = function (letter, pro) {
    ...
    ul = '<ul class="city-picker animated bounceInLeft">' + li + '</ul>';
  ...
  p.proClick = function () {
    $('.pro-picker').on('click', function (e) {
      var target = e.target;
      if ($(target).is('dd')) {
        this.pro = $(target).html();
        var letter = $(target).data('letter');
        this.createCityList(letter, this.pro);
        this.forward();
        $(this).hide();
      }
    }.bind(this));
  ...
  p.cityClick = function () {
    $('.city-picker').on('click', function (e) {
      var target = e.target;
      if ($(target).is('li')) {
        this.city = $(target).html();
        if (this.elType) {
          this.el.val(this.pro + '-' + this.city);
        } else {
          this.el.html(this.pro + '-' + this.city);
        }
        console.info(CityPicker.city);
        $('.picker-box').hide();
        $('.pro-picker').hide();
        $(this).hide();
  ...
  p.proPickerEvent = function () {
    var _start = 0;
    var _end = 0;
    var proPicker = $('.pro-picker');
    proPicker.on('touchstart', function (event) {
      var touch = event.targetTouches[0];
      _start = touch.pageY;
    });
    proPicker.on('touchmove', function (event) {
      var touch = event.targetTouches[0];
      _end = (_start - touch.pageY) / 2;
      var scrollTop = event.currentTarget.scrollTop;
      if (_end < 0 && scrollTop <= 0) {
        event.preventDefault();
      }
    });
    proPicker.on('touchend', function (event) {

    });
  };


  p.cityPickerEvent = function () {
    var _start = 0;
    var _end = 0;
    var cityPicker = $('.city-picker');
    cityPicker.on('touchstart', function (event) {
      var touch = event.targetTouches[0];
      _start = touch.pageY;
    });
    cityPicker.on('touchmove', function (event) {
      var touch = event.targetTouches[0];
      _end = (_start - touch.pageY) / 2;
      var scrollTop = event.currentTarget.scrollTop;
      if (_end < 0 && scrollTop <= 0) {
  ...
  p.eventListener = function (target, eventType, callback) {
